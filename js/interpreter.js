...
parse(){
	const lines=this.m3u8Content.replace('\r','').replace(/(\n{2,})/g,'\n\n').split('\n');
	let currentChannel=null;
	let firstLine=lines[0].trim();
	if(firstLine.startsWith('#EXTM3U')){
		this.parseEPGInfo(firstLine)
		}
		
lines.forEach(line=>{line=line.trim();
if(line.startsWith('#EXTINF:')){
	const channelInfo=this.parseChannelInfo(line);
	currentChannel={...channelInfo,licenseType:null,licenseKey:null,manifestUrl:null}
	}
	else if(line.startsWith('#KODIPROP:')){
		this.parseKodiprop(line,currentChannel)
		}
	else if(/^[A-Za-z0-9]+:\/\//.test(line)&&currentChannel){
		currentChannel.manifestUrl=line;
		this.channels.push(currentChannel);
		currentChannel=null
		}
		});
		if(currentChannel&&currentChannel.manifestUrl){
			this.channels.push(currentChannel)}
			}

extractUrls(myString){
...
parseEPGInfo(line){const regexUrlTvg=/url-tvg="([^"]+)"/
	...
	this.urlTvg=regexUrlTvg.exec(line)?regexUrlTvg.exec(line)[1]:null;
	this.urlTvg=this.extractUrls(this.urlTvg);
	this.refresh=regexRefresh.exec(line)?regexRefresh.exec(line)[1]:null
	}
parseChannelInfo(line){
	const regex=/tvg-id="([^"]+)"\s+group-title="([^"]+)"\s+group-logo="([^"]+)"\s+tvg-logo="([^"]+)"/;
	...
	try{
		return{tvgId:regexTvgId.exec(line)?regexTvgId.exec(line)[1]:null,tvgLogo:regexTvgLogo.exec(line)?regexTvgLogo.exec(line)[1]:null,groupTitle:regexGroupTitle.exec(line)?regexGroupTitle.exec(line)[1]:null,groupLogo:regexGroupLogo.exec(line)?regexGroupLogo.exec(line)[1]:null,channelName:channelName}}catch{}
	return{tvgId:null,tvgLogo:null,groupTitle:null,groupLogo:null,channelName:channelName}}

parseKodiprop(line,currentChannel){
	if(!currentChannel)return;if(line.startsWith('#KODIPROP:inputstream.adaptive.license_type=')){currentChannel.licenseType=line.split('=')[1].trim()}else if(line.startsWith('#KODIPROP:inputstream.adaptive.license_key=')){const licenseKey=this.parseLicenseKey(line);currentChannel.licenseKey=licenseKey}}
